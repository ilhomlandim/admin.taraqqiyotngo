...
import { Pencil, Trash2, Image, X, Calendar, Globe, MessageSquare, ChevronDown, FileText, AlertCircle } from "lucide-react";
      ...
      <div className="rounded-xl shadow-lg overflow-hidden border">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {data.map((item) => (
                <tr key={item.id} className="border-t hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                    ...
                    (item.title.uz || item.title.ru || item.title.en) ? (
                      ...
                      >
                        <div className="font-medium truncate flex items-center gap-2">
                          <Globe size={16} className="opacity-50 group-hover:opacity-100 transition-opacity" />
                          {item.title.uz || item.title.ru || item.title.en}
                        </div>
                      </button>
                    ) : (
                        ...
                        item.images.map((img, idx) => (
                          ...
                          >
                            ...
                            <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-30 transition-all rounded-lg flex items-center justify-center">
                              <Image className="opacity-0 group-hover:opacity-100 text-white" size={18} />
                            </div>
                          </button>
                        ))
                      ) : (
